...
las preferencias suaves y los factores de arriba (Tafira vs. fallback, estado de
la vivienda, calificación energética, proximidad) y contra lo que esa vivienda
realmente vale en el mercado de Las Palmas de Gran Canaria. Ningún factor
individual manda sobre los demás — es una valoración de conjunto, igual que
pediría la familia si mirara los anuncios ella misma. No hay requisito de
despacho/zona de escritorio: no lo uses ni para eliminar ni para desempatar.
...
jardín privado/comunitario**, así que hace falta abrir cada anuncio serio (paso 3)
igualmente.
...
Idealista (DataDome) y devuelve 403/CAPTCHA en el 100% de los casos — pasó en
toda la ejecución del 2026-08-28, y ese día los 5 ganadores acabaron con el
jardín "confirmado" a base de suposición, no de lectura real, porque no había
otra fuente. Para cualquier ficha de idealista.com, usa por Bash (ruta absoluta
— tu directorio de trabajo aquí es un scratch dir sin los scripts del repo, así
que una ruta relativa como `scripts/idealista_detail.py` no existe):
```bash
...
atraviesa el bloqueo. Si aun así falla (p. ej. "session expired" porque la
sesión de Idealista caducó), trátalo exactamente como un dato no confirmable:
dilo en `flags` y no marques `has_garden: true` por intuición — un jardín no
confirmado no pasa el filtro (ver regla más abajo). Fotocasa y pisos.com sí se
abren con WebFetch con normalidad, no tienen este bloqueo.
...
[
  {
    ...
    "score": 87,
    "is_target_area": true,
    "verdict": "Dos o tres frases en español. Por qué gana: jardín privado real de X m², precio frente al mercado de Tafira, y el pero más importante.",
